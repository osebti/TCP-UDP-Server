...
void server(){
    ...
    while (1){  
        ...
        if(sendMsg.type == ACK_TYPE){
            printf ("Transmitted: ACK(d) In", sendMsg.seqNum);
        }

        else{
            printf("Transmitted: NAK(%d)n", sendMsg.seqNum);
        }
...
void client (const char *serverName, const char *dataFile, int nLine){
    ...
    if (server==NULL){
        fprintf(stderr,"ERROR, no such hostln");
        exit(0);
    }
...
int main(int argc, char *argv[]){
    ...
    if(argc<3){
        ...
        fprintf(stderr, "Usage for Client: %s -c serverName dataFile nLinel\n",argv[0]);
        exit(1);
