...
int Delete(char* object_name,int id){
    ...
    for(int i=0;i<16;i++){
        if(strcmp(table[i].object_name,object_name)==0){ // empty block 
            if(table[i].owner!=id){
                s_frame.kind=6;
...
void server(int port){
    ...
    while(1){
        ...
        if((ret=poll(pfds,N,timeout)) < 0){ // poll error 
            perror("poll: ");
        } 

        if ( (N < NCLIENT+2) && (pfds[0].revents && POLLIN)) {
